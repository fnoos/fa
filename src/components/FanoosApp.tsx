...
export default function FanoosApp({ posts }: { posts: any[] }) {
  ...
  const dynamicCategories = useMemo(
    () => ['الكل', ...Array.from(new Set(posts.map(p => p.category).filter(Boolean)))],
    [posts]
  );
  ...
  const filteredPosts = useMemo(() => {
    return posts.filter(p => {
      ...
      return selectedCategory === 'الكل' || postCat === targetCat;
    });
    ...
    <main style={{ 
          ...
          >
            ...
            >
              <div style={{ textAlign: 'right', direction: 'rtl' }}>
                <h2 style={{ color: themeColor, fontWeight: '900', fontSize: '24px', marginBottom: '15px', textAlign: 'center' }}>معلومات عن فانوس</h2>
                <strong style={{ fontSize: '18px', display: 'block', marginBottom: '10px', fontWeight: '900', color: themeColor }}>فانوس | رفيق ليالي التردد 🌙</strong>
                
                <div style={{ color: isDarkMode ? '#eee' : '#333', lineHeight: '1.7', fontSize: '15px', textAlign: 'justify' }}>
                  <p style={{ margin: '0 0 10px 0' }}>
                    كلنا مررنا بليالٍ تسللت فيها الأسئلة إلى أرواحنا مثل دودة تنخر الفؤاد. هنا لن يُقدَّم لك وعظ، ولن تُحاصر بكلماتٍ ثقيلة. نحن فقط رفعنا فانوسًا لنميز معًا الطريق من الضلال، والحقيقة من الأداء المصطنع.
                  </p>
                  <p style={{ margin: '0 0 10px 0' }}>
                    قلبُك أكبر من أن يبقى في قفص «العبث».
                  </p>
                  <p style={{ margin: '0' }}>
                    هيا معًا نبدأ من جديد. 🤍
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <button onClick={() => setOpenDrawer(null)} style={{ backgroundColor: themeColor, color: '#fff', border: 'none', padding: '12px', borderRadius: '20px', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', width: '100%' }}>إغلاق</button>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <div style={{ padding: '40px 10px 10px', textAlign: 'center', position: 'relative', maxWidth: '480px', margin: '0 auto' }}>
        <div style={{ position: 'absolute', left: '13px', top: '48px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <div style={{ position: 'relative' }} onMouseEnter={() => setHoveredIcon('about')} onMouseLeave={() => setHoveredIcon(null)}>
            ...
            {hoveredIcon === 'about' && (
              <div style={{ position: 'absolute', left: 'calc(100% + 10px)', top: '50%', transform: 'translateY(-50%)', backgroundColor: themeColor, color: '#fff', padding: '5px 12px', borderRadius: '8px', fontSize: '13px', fontFamily: "'Estedad', sans-serif", whiteSpace: 'nowrap', zIndex: 1000, pointerEvents: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
      ...
      <div style={{ position: 'fixed', bottom: '30px', left: '50%', transform: 'translateX(-50%)', width: '350px', zIndex: 100 }}>
        ...
        <nav style={{ backgroundColor: isDarkMode ? 'rgba(30,30,30,0.9)' : 'rgba(255,255,255,0.95)', height: '75px', borderRadius: '28px', display: 'flex', alignItems: 'center', boxShadow: '0 15px 40px rgba(0,0,0,0.1)' }}>
          <div onClick={() => { scrollToTop(); setOpenDrawer(null); }} style={navItem}><IconHome color={(selectedCategory === 'الكل' && searchQuery === '') ? themeColor : '#999'} /><span style={{...navText, color: (selectedCategory === 'الكل' && searchQuery === '') ? themeColor : '#999'}}>الرئيسية</span></div>
          <div onClick={() => setOpenDrawer(openDrawer === 'cat' ? null : 'cat')} style={navItem}><IconGrid color={(openDrawer === 'cat' || (selectedCategory !== 'الكل' && searchQuery === '')) ? themeColor : '#999'} /><span style={{...navText, color: (openDrawer === 'cat' || (selectedCategory !== 'الكل' && searchQuery === '')) ? themeColor : '#999'}}>التصنيف</span></div>
          <div onClick={() => setOpenDrawer(openDrawer === 'color' ? null : 'color')} style={navItem}><IconColor color={openDrawer === 'color' ? themeColor : '#999'} /><span style={{...navText, color: openDrawer === 'color' ? themeColor : '#999'}}>اللون</span></div>
          <div onClick={() => { setIsDarkMode(!isDarkMode); setOpenDrawer(null); }} style={navItem}>{isDarkMode ? <IconSun /> : <IconMoon />}<span style={{...navText, color: '#999'}}>المظهر</span></div>
          <div onClick={() => { setFontSize(fontSize >= 28 ? 16 : fontSize + 3); setOpenDrawer(null); }} style={navItem}><div style={{ width: '28px', height: '28px', backgroundColor: themeColor, borderRadius: '50%', color: '#fff', display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '11px', fontWeight: '900' }}>Aa</div><span style={{...navText, color: '#999'}}>الحجم</span></div>
        </nav>
      </div>
    </main>
  );
...
function AudioPlayer({ src, themeColor, isDarkMode }: any) {
  ...
  const handleShareAudio = (e: any) => {
    e.stopPropagation();
    if (navigator.share) {
      navigator.share({ title: '', text: '🎧 صوت من فانوس', url: src });
    } else {
      navigator.clipboard.writeText(src);
      const fb = document.createElement('div');
      fb.innerText = 'تم نسخ رابط الصوت';
      fb.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#333;color:#fff;padding:10px 20px;border-radius:14px;font-size:13px;font-weight:700;z-index:999;font-family:Estedad,sans-serif;direction:rtl';
  ...
  const Tip = ({ text }: any) => (
    <div style={{ position: 'absolute', bottom: 'calc(100% + 6px)', left: '50%', transform: 'translateX(-50%)', backgroundColor: isDarkMode ? '#444' : '#333', color: '#fff', padding: '5px 10px', borderRadius: '8px', fontSize: '11px', fontWeight: '700', whiteSpace: 'nowrap', zIndex: 20, fontFamily: 'Estedad, sans-serif', direction: 'rtl' }}>
      {text}
    </div>
  );
    ...
    <div onClick={(e) => e.stopPropagation()} style={{ marginTop: '14px', backgroundColor: isDarkMode ? '#2a2a2a' : '#f5f5f5', borderRadius: '18px', padding: '14px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div onClick={togglePlay} onMouseEnter={() => setTip('play')} onMouseLeave={() => setTip(null)} style={{ width: '40px', height: '40px', borderRadius: '50%', backgroundColor: themeColor, display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer', flexShrink: 0, boxShadow: `0 4px 12px ${themeColor}40`, position: 'relative' }}>
          ...
          {tip === 'play' && <Tip text={playing ? 'إيقاف' : 'تشغيل'} />}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div onClick={(e) => { e.stopPropagation(); setShowSpeed(!showSpeed); }} onMouseEnter={() => setTip('speed')} onMouseLeave={() => setTip(null)} style={{ cursor: 'pointer', fontSize: '12px', fontWeight: '700', color: isDarkMode ? '#aaa' : '#777', padding: '4px 6px', borderRadius: '8px', backgroundColor: isDarkMode ? '#3a3a3a' : '#e8e8e8', position: 'relative' }}>
            {playbackRate}x
            {tip === 'speed' && <Tip text="سرعة التشغيل" />}
            {showSpeed && (
          ...
          <div onClick={handleShareAudio} onMouseEnter={() => setTip('share')} onMouseLeave={() => setTip(null)} style={{ cursor: 'pointer', flexShrink: 0, position: 'relative', display: 'flex' }}>
            ...
            {tip === 'share' && <Tip text="مشاركة الصوت" />}
          </div>
...
function PostCard({ post, fontSize, themeColor, isDarkMode, setSearchQuery }: any) {
  const readingTimeText = useMemo(() => {
    ...
    return seconds < 60 ? `${seconds} ثانية قراءة` : `${Math.ceil(seconds / 60)} دقيقة قراءة`;
  }, [post.content]);

  const handleShare = async (e: any) => {
    e.stopPropagation();
    const plainText = post.content.replace(/<[^>]*>/g, '').trim();
    const shareMessage = `${plainText}\n\n✨ فـانـوس\n---------------------------\nانضموا إلينا هنا:\n\nتلغرام: t.me/fanoosarea\nانستغرام: instagram.com/fanoosarea\nالموقع: https://fnoos.vercel.app`;
    try {
  ...
  const showFeedback = (event: any) => {
    ...
    feedback.innerText = 'تم نسخ النص وجاهز للإرسال';
